...
CREATE TABLE IF NOT EXISTS public.avwap_futures_3m (
    ...
    high_cross TEXT,
    low_cross TEXT,
    price_cross_above_avwap BOOLEAN NOT NULL DEFAULT FALSE,
    price_cross_below_avwap BOOLEAN NOT NULL DEFAULT FALSE,
    price_avwap_state TEXT,
    price_distance_avwap_high_pct NUMERIC,
    price_distance_avwap_low_pct NUMERIC,
...
ALTER TABLE public.avwap_futures_3m ADD COLUMN IF NOT EXISTS price_cross_above_avwap BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE public.avwap_futures_3m ADD COLUMN IF NOT EXISTS price_cross_below_avwap BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE public.avwap_futures_3m ADD COLUMN IF NOT EXISTS price_avwap_state TEXT;
ALTER TABLE public.avwap_futures_3m ADD COLUMN IF NOT EXISTS price_distance_avwap_high_pct NUMERIC;
ALTER TABLE public.avwap_futures_3m ADD COLUMN IF NOT EXISTS price_distance_avwap_low_pct NUMERIC;

CREATE INDEX IF NOT EXISTS idx_avwap_futures_price_crosses
    ON public.avwap_futures_3m (trading_date, candle_start DESC)
    WHERE price_cross_above_avwap OR price_cross_below_avwap;
...
INSERT INTO public.avwap_futures_3m (
    ...
    price_cross_above_avwap, price_cross_below_avwap, price_avwap_state,
    price_distance_avwap_high_pct, price_distance_avwap_low_pct
) VALUES (
    ...
    %(price_cross_above_avwap)s, %(price_cross_below_avwap)s, %(price_avwap_state)s,
    %(price_distance_avwap_high_pct)s, %(price_distance_avwap_low_pct)s
)
ON CONFLICT (trading_date, instrument_key, candle_start) DO UPDATE SET
    ...
    price_cross_above_avwap = EXCLUDED.price_cross_above_avwap,
    price_cross_below_avwap = EXCLUDED.price_cross_below_avwap,
    price_avwap_state = EXCLUDED.price_avwap_state,
    price_distance_avwap_high_pct = EXCLUDED.price_distance_avwap_high_pct,
    price_distance_avwap_low_pct = EXCLUDED.price_distance_avwap_low_pct,
    updated_at = NOW();
"""
...
class Collector:
    ...
    def restore_today(self, trading_date: date) -> None:
        ...
        SELECT DISTINCT ON (instrument_key)
            ...
            hourly_avwap_high, hourly_avwap_low, close, candle_start
        ...
        with db_connect() as conn:
            with conn.cursor() as cur:
                ...
                for key, cum_vol_end, avh, avl, hour_h, hour_l, last_close, _ in cur.fetchall():
                    state = self.states.get(key)
                    if state:
                        ...
                        state.previous_close = float(last_close) if last_close is not None else None
    ...
    def finalise_bar(self, state: SymbolState) -> None:
        ...
        price_cross_above = False
        price_cross_below = False
        price_state = None
        dist_high_pct = ((bar.close / avwap_high) - 1.0) * 100.0 if avwap_high else None
        dist_low_pct = ((bar.close / avwap_low) - 1.0) * 100.0 if avwap_low else None
        if bar.start >= freeze_at and state.previous_close is not None:
            # Bullish price trigger: close crosses from at/below the continuing High-source AVWAP to above it.
            if state.previous_avwap_high is not None and avwap_high is not None:
                price_cross_above = state.previous_close <= state.previous_avwap_high and bar.close > avwap_high
            # Bearish price trigger: close crosses from at/above the continuing Low-source AVWAP to below it.
            if state.previous_avwap_low is not None and avwap_low is not None:
                price_cross_below = state.previous_close >= state.previous_avwap_low and bar.close < avwap_low
        if avwap_high is not None and bar.close > avwap_high:
            price_state = "ABOVE_AVWAP_HIGH"
        elif avwap_low is not None and bar.close < avwap_low:
            price_state = "BELOW_AVWAP_LOW"
        elif avwap_high is not None and avwap_low is not None:
            price_state = "BETWEEN_AVWAPS"
        ...
        row = {
            ...
            "low_cross": low_cross,
            "price_cross_above_avwap": price_cross_above,
            "price_cross_below_avwap": price_cross_below,
            "price_avwap_state": price_state,
            "price_distance_avwap_high_pct": dist_high_pct,
            "price_distance_avwap_low_pct": dist_low_pct,
        }
        self.write_queue.put(row)
        self.last_bar_at = bar_end
        if high_cross or low_cross or price_cross_above or price_cross_below:
            LOG.info(
                "%s avwap_ref_high=%s avwap_ref_low=%s price_above=%s price_below=%s at %s",
                state.instrument.symbol, high_cross, low_cross, price_cross_above, price_cross_below, bar_end,
            )
...
def wait_until_subscription_window() -> None:
    """Keep Railway alive and connect only on valid NSE F&O trading sessions."""
    last_notice = None
    while True:
        now = datetime.now(IST)
        today = now.date()
        connect_at = market_datetime(today, dt_time(9, 10))
        close_at = market_datetime(today, MARKET_CLOSE)
        ...
        if not is_nse_trading_day(today):
            ...
            if notice != last_notice:
                LOG.info("NSE session closed on %s (%s); next AVWAP connection %s 09:10 IST", today, reason, next_day)
                last_notice = notice
            time.sleep(min(900, max(1, int((next_connect - now).total_seconds()))))
            continue
        if now > close_at:
            next_day = next_nse_trading_day(today + timedelta(days=1))
            next_connect = market_datetime(next_day, dt_time(9, 10))
